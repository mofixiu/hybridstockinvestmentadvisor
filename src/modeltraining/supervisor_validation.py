...
def run_supervisor_validation():
    print("🎓 Starting Supervisor Validation Protocol (5-Model Generalization)...")
    ...
    # 4. Define 5 Generalized Models (As requested by Supervisor)
    models = {
    ...
    # 5. Train All Models on the Global Dataset
    print("\n🧠 Training 5 models on generalized multi-stock data...")
    for name, model in models.items():
        model.fit(X_train, y_train)
    ...
    # Generate Chart for Chapter 4 (Updated for 5 bars without warnings)
    metrics_df = pd.DataFrame(metrics_list)
    plt.figure(figsize=(10, 6)) # Made slightly wider to fit 5 names cleanly
    
    # Using hue="Model" and legend=False fixes the seaborn deprecation warning
    ax = sns.barplot(x="Model", y="Score", hue="Model", data=metrics_df, palette="viridis", legend=False)
    
    plt.title("Model Generalization Test: F1-Scores on GTCO (5 Models)", fontsize=14, fontweight='bold')
    plt.ylim(0, 1.0)
